...
//Box2d Code
    ...
    Line debug(vec3(x_s, y_s, 0.0f), vec3(x_e, y_e, 0.0f), SCR_WIDTH, SCR_HEIGHT);
...
//	x_cam = s->position.x;	
	...
	//o_cam->SetPosition(glm::vec3(s->getPosition().x - 100, s->getPosition().y - 100, 0));
...
/*
			...
			*/	
			...
			debug.draw(x_line, y_line, s->angle);
