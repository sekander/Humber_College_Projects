...
Quaternion MMath::toQuaternion(const Matrix3& m) {
	Quaternion result;
	/// Two options depending on the sign of the trace
	/// Remember Dr. Fielder's wise words:
	...
	const float trace = R11 + R22 + R33;
	if (trace > 0.0f) {
		// If trace is positive, then no trouble dividing by w which is > 1/2
		result.w = 0.5f * sqrt(trace + 1.0f);
		const float denom = 4.0f * result.w;
		result.ijk.x = (R32 - R23) / denom;
		result.ijk.y = (R13 - R31) / denom;
		result.ijk.y = (R21 - R12) / denom;

	}
	else {
		/// w might not be a suitable divisor as it could approach zero
		/// Examine the diagonal entries in the quaternion quadratic form
		const float qx = 0.5f * sqrt(1.0f + R11 - R22 - R33);
		const float qy = 0.5f * sqrt(1.0f + R22 - R33 - R11);
		const float qz = 0.5f * sqrt(1.0f + R33 - R11 - R22);

		if (fabs(qx) > fabs(qy) && fabs(qx) > fabs(qz)) {
			const float denom = 4.0f * qx;
			result.w = (R32 - R23) / denom;
			result.ijk.x = qx;
			result.ijk.y = (R21 + R12) / denom;
			result.ijk.z = (R13 + R31) / denom;
		}
		else if (fabs(qy) > fabs(qx) && fabs(qy) > fabs(qz)) {
			const float denom = 4.0f * qy;
			result.w = (R13 - R31) / denom;
			result.ijk.x = (R12 + R21) / denom;
			result.ijk.y = qy;
			result.ijk.z = (R23 + R32) / denom;
		}
		else {
			const float denom = 4.0f * qz;
			result.w = (R21 - R12) / denom;
			result.ijk.x = (R31 + R13) / denom;
			result.ijk.y = (R23 + R32) / denom;
			result.ijk.z = qz;
		}
	}
	return result;
